...
subroutine rhs_w
	...
	use user_params,                     only: eval_rho_bar
	implicit none
	integer,parameter                       :: id=3
	integer                                 :: k
	integer, save                           :: locnz
	real(kind=8)                            :: dz,gamma,rho_bar
	real(kind=8), allocatable, save         :: z_taper(:),z(:)
	...
	if( scalar_kind(1) =='r' ) then
		do k=1,locnz
			rho_bar = eval_rho_bar(z(k))			
	 		explicit_rhs(:,:,k,id,MM0) = explicit_rhs(:,:,k,id,MM0) - (g/rho0)*(s1(:,:,k)-rho_bar) 
	 	enddo   
	endif

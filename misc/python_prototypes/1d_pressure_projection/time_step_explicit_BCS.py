...
"""
  ...
  all dependent variables
    ...
    u_t = A*cos(2pi/L x) - p_x    u_initial = 1
    ...
    u* = u_n + body_force    in the interior;  BV(t) at x=0,L   bdry vals BV obtained externally
    div u* = d/dx u* 
...
gamma = 5.*dx    		# [m]           decay length for "bending" near boundaries  (require gamma<<L)
...
A = 0.0                           # [m/s2]  amplitude of body force
body_force = A*np.cos((2.*pi/L)*x)  #
frac=0.05                           # filtering fraction for d/dx of u* and p   0.10
...
for j in range(nt):
	...
	ustar = u_n + dt*( body_force + diss )    #  body force that generates divergence that needs elimination
	ustar[0]  = u_BV
	ustar[-1] = u_BV                   # impose u_n+1 BVALS on u* itself
	
	u_exact = u_BV*np.ones_like(x)


	#--------------------------------------------------------------------------------------------------
	#  adjust end values to match BCS on u at tnp1, spread" jump to small but finite transition region
	#--------------------------------------------------------------------------------------------------
	for i in range(nx):
		if( x[i] <= L/2. ):
			g = 1.0 - np.exp(-((x[i]-a)/gamma)**4)
			ustar[i] = (ustar[i]-ustar[0])*g + ustar[0]
		else:
			g = 1.0 - np.exp(-((x[i]-b)/gamma)**4)
	...
	if( j==plot_step):
		...
		title_string = r"$u^*$ with bdry values imposed and smoothed, $\gamma=5\Delta x$"    
	...
	if( j==plot_step):
		...
		axes.plot(x_ext,p_ext,'k.',markersize=3)

...
for islice in range(0,1025,16): 
	...
	varname = 'u'
	u = read_netcdf_2d_var(varname,filename,0)    # u[z,x]

	U = np.zeros( (NZ,NX), float )
	for i in range(NX):
		for k in range(NZ):
			varid = 0  # U
			U[k,i] = parent_soln(X[i],Z[k],t,varid)
	...
	title_string = r"$u(x,z,t)$   t=%.2f IW periods " %(t/IWPER)

...
#     p=poisson_invert(f,L)             solve p_xx = f  assuming f can be even extended across x=L, BCs p_x=0 at x=0,L
#
#     the 3 routines above were modified to return some additional intermediate arrays/results to facilitate
#     plotting the behavior of the spectral approach to boundary forced problems
#
...
#-----------------------------------------------------------------------------------------
    ...
    if flag == 0:                                            #  Fourier series
     ...
     FHAT = ((i*k)**n) * fft(F)                              # have imported scipy's fft,ifft
     df = ifft(FHAT)[0:M/2+1].real
     df_ext = np.concatenate([df, df[1:-1][::-1]])          # even extension of df
    else:

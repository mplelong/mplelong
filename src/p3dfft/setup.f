C ------------------------------------------------------------
      ...
      !  KW
      integer, allocatable :: ST_ARRAY(:),EN_ARRAY(:),SZ_ARRAY(:)
      !  KW
...
! KW
!  what is returned is 3 integer arrays of length M=proc_grid(j,s) that should be stored in
!  st(s)%R(0:M-1,i,j), en(s)%R(0:M-1,i,j), and sz(s)%R(0:M-1,i,j)
      allocate (ST_ARRAY(0:pm-1))
      allocate (SZ_ARRAY(0:pm-1))
      allocate (EN_ARRAY(0:pm-1))
! KW

      do i=1,3
         do j=1,2

! original code... allocatable structures in subroutine argument list
!           call MapDataToProc(dims(i),proc_grid(j,s),st(s)%R(0,i,j),
!    &            en(s)%R(0,i,j), sz(s)%R(0,i,j))

!KW  to get rid of gcc complaint/error
            call MapDataToProc(dims(i),proc_grid(j,s),ST_ARRAY(0),
     &            EN_ARRAY(0), SZ_ARRAY(0) )

            st(s)%R(0:proc_grid(j,s)-1,i,j) = ST_ARRAY(0:proc_grid(j,s)-1)
            en(s)%R(0:proc_grid(j,s)-1,i,j) = EN_ARRAY(0:proc_grid(j,s)-1)
            sz(s)%R(0:proc_grid(j,s)-1,i,j) = SZ_ARRAY(0:proc_grid(j,s)-1)
!KW  to get rid of gcc complaint/error

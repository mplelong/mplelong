...
subroutine rhs_w
	...
	use user_params,                     only: rho_south,rho_north
	implicit none
	integer,parameter                       :: id=3
	integer                                 :: k
	integer, save                           :: locnz
	real(kind=8)                            :: rho_mean  
	...
	if( scalar_kind(1) =='r' ) then
		do k=1,locnz
			rho_mean = ( rho_north(k) + rho_south(k) )/2.d0    ! these are local arrays
			explicit_rhs(:,:,k,id,MM0) = explicit_rhs(:,:,k,id,MM0) - (g/rho0)*(s1(:,:,k) - rho_mean)
		enddo   
	endif

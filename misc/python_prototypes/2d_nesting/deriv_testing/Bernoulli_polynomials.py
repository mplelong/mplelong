...
#--------------------------------------------------------------------------------------
	...
	c = -1.                                        #c = -(P)**n / fac(n+1)
	if( xsi < 0 ): 
		xsi = 1. + xsi	                           #  wrap xsi as necessary for periodicity
	elif( xsi > 1 ):
		xsi = xsi - 1. 
	if( x==P/2. and a==P/2. ): xsi = 1.0	       # endpoint should have xsi=1 not xsi = 0
	ans = c*B_n(xsi,n+1)					
	return ans
...
#--------------------------------------------------------------------------------------
	...
	if( n >= k ):
		ddxsi = c * fac(n+1)/fac(n-k+1) * B_n(xsi,n+1-k)  # kth deriv of U_n(xsi) wrt to xsi
...
def setup_factor_matrix(x,Q,x0,inc):
#--------------------------------------------------------------------------------------
...
#    constrain the Q term expansions to match f at M = (Q+1)/2 points near 0 and L.
#    The matching ponts are separated by inc grid points.
#--------------------------------------------------------------------------------------
	import numpy as np
	from scipy.linalg import lu_factor
	
	nx = x.size
	M = (Q+1)/2  ;  A = np.zeros((M,M))
	
	if(x0==x[0]):
		istart = 0                     # array index of first x val to include
	elif(x0==x[-1]): 
		istart = (nx-1) - inc*(M-1)    # array index of first x val to include  nx-M
	else:
	...
	for i in range(M):                    # matrix rows, x location varies
		xval = x[istart+i*inc]
		for j in range(M):                # matrix cols, odd polynomial order varies
			n = 2*j+1
			A[i,j] = U_n(xval,a,P,n)
	
	# decompose Matrix into LU		
	lu,piv = lu_factor(A)
	return lu,piv
	
def solve_for_expansion_coeffs(lu,piv,f,Q,x0,inc):
#--------------------------------------------------------------------------------------
...
#    should match f at the (Q+1)/2 points near x=0, separated by inc grid points.
#    if the expansion point x0=L, then the coeffs are B1,B3,..B_Q and the series
#    should match f at the M=(Q+1)/2 points near x=L, separated by inc grid points.
#--------------------------------------------------------------------------------------
	import numpy as np

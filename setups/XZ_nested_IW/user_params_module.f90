...
module user_params
	...
	real(kind=8), parameter          :: mode_x = 1     !! periodic mode number in L
	real(kind=8), parameter          :: mode_z = 1     !! cos mode number in H
	real(kind=8), parameter          :: x0 = 0.5d0*L   !! offset for corner of computational domain
	real(kind=8), parameter          :: y0 = 0.d0      !! offset for corner of computational domain
...
contains
	...
	!-----------------------------------------------------------------
		...
		real(kind=8)                :: pi,argx,argy,argz,ans
		...
		if( first_entry ) then
			...
			f2 = f0*f0 
			N2 = N*N
			omega2 = (kx**2*N2 + kz**2*f2)/(kx**2 + kz**2)
			omega = sqrt(omega2)
			phase = pi/4.d0			
			first_entry = .FALSE.
		endif
		
		argx = kx*X - omega*t + phase
		argz = kz*Z
		
		if(id==1) ans = A*cos(argz)*cos(argx)                              ! U
		if(id==2) ans = A*(f0/omega)*cos(argz)*sin(argx)                   ! V
		if(id==3) ans = A*(kx/kz)*sin(argz)*sin(argx)                      ! W
				
		if( id==4 ) then
			ans = -A*(kx/kz)*(N2/omega)*sin(argz)*cos(argx)                ! B
			ans = -(rho0/g)*ans  
		endif
		
		if(id==6) ans = -A*(1./(kx*omega))*(f2-omega2)*cos(argz)*cos(argx) ! P
	...
	!-----------------------------------------------------------------
		...
		if(id==1) then         ! U = A*cos(argz)*cos(argx)
			if(dir=='x') ans= -kx * A*cos(argz)*sin(argx)
			if(dir=='y') ans= 0.d0
			if(dir=='z') ans= -kz * A*sin(argz)*cos(argx)
		elseif(id==2) then     ! V = A*(f0/omega)*cos(argz)*sin(argx)
			if(dir=='x') ans=  kx * A*(f0/omega)*cos(argz)*cos(argx)
			if(dir=='y') ans=  0.d0
			if(dir=='z') ans= -kz * A*(f0/omega)*sin(argz)*sin(argx)
		elseif(id==3) then     ! W = A*(kx/kz)*sin(argz)*sin(argx)
			if(dir=='x') ans=  kx * A*(kx/kz)*sin(argz)*cos(argx)
			if(dir=='y') ans=  0.d0
			if(dir=='z') ans=  kz * A*(kx/kz)*cos(argz)*sin(argx)
		elseif(id==4) then     ! B = -A*(kx/kz)*(N2/omega)*sin(argz)*cos(argx)
			if(dir=='x') ans=  kx * A*(kx/kz)*(N2/omega)*sin(argz)*sin(argx)
			if(dir=='y') ans=  0.d0
			if(dir=='z') ans= -kz * A*(kx/kz)*(N2/omega)*cos(argz)*cos(argx)

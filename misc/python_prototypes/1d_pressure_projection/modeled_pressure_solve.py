...
"""
  ...
  all dependent variables
     ...
     p_xx = d/dx(u*)        u* = cos(pi*x/L)  d/dx u* = -(pi/L)*sin(pi*x/L)
...
ustar = np.cos(2*pi*x/L)            # [1] d/d u* is rhs for pressure eqn
...
p_exact = (L/(2.*pi))*np.sin((2.*pi/L)*x) - x + 1./2. #  closed form
...
title_string = r"$\frac{1}{\pi} u^*_x$"    
axes.set_title(title_string,fontsize=12)
axes.plot(x/L,div/pi,'k',linewidth=1)
axes.plot(x_ext/L,div_ext/pi,'k',linewidth=1)
axes.plot(x/L,div/pi,'r.',markersize=6)
axes.plot(x_ext/L,div_ext/pi,'k.',markersize=4)
axes.grid()
...
title_string = r"computed soln (k,r), exact soln (b),  scaled, extended rhs (g)"    
axes.set_title(title_string,fontsize=12)
...
axes.plot(x/L,np.cos((2.*pi/L)*x)-1.0,'ko',markersize=3)
axes.plot(x/L,ustar,'k--',linewidth=1)

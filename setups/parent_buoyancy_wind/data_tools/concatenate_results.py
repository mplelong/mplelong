...
ES = 55           # end_slice XY XZ YZ planes   (start and inc assumed to be 1)
NUMPROCS_2D = 5   # num procs to share the workload

ES_3D = 880         # end slice for stitching the distributed 3D files (start assumed to be 1)
INC_3D = 220        # inc between 3D saves
NUMPROCS_3D = 5     # num procs to share the workload

do_child_grid = False                         # whether to concatenate child grid files as well
...
if( do_XZ ):
	...
	numprocs = NUMPROCS_2D                  # number of processors to be used by concat_XZ_mpi.py	
...
if( do_YZ ):
	...
	numprocs = NUMPROCS_2D                  # number of processors to be used by concat_YZ_mpi.py	
...
if( do_XY ):
	...
	numprocs = NUMPROCS_2D                  # number of processors to be used by concat_XY_mpi.py	
...
if( do_XYZ ):
	...
	start_slice = 0                         # starting time slice  i.e. slices = np.arange(start_slice,end_slice,inc)
	end_slice = ES_3D                       # ending time slice    
	inc = INC_3D                            # time slice increment
	numprocs = NUMPROCS_3D                  # number of processors to be used by concat_YZ_mpi.py

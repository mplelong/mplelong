...
slice_dir = top_dir + 'data_from_outer_run/'         # where the H & V interpolated global 3d file lives
restart_dir = top_dir + 'nested_run/8x20/RESTART/'   # where the p1xp2 restart files will be written, created if necessary
slice_index = 99999999999                    # slice index of input file, not needed if input generated by interp_V.py
islice = 0                                   # typically only 1 time slice per file
do_second_scalar=False                       # s2 present in original file??
from_interpolated=True                       # use file naming convention of interp_V.py, i.e. input is slices/3D/vert_horiz_interp.nc


#---------------------------------------------------------------------------------------
#  desired decomposition parameters  p1<-->nx,  p2<-->ny,nz   12x12=144   4x10=40
#---------------------------------------------------------------------------------------
p1 = 8                    
p2 = 20               # decomposition parameters to be used in nested flow_solve run  
...
#---------------------------------------------------------------------------------------
fname = slice_dir + 'XYZ_' + str(slice_index).zfill(6) + '.nc'
if( from_interpolated ): fname = slice_dir + 'vert_horiz_interp.nc'

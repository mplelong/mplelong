...
subroutine user_ics(x,y,z,F,id,nx,ny,nz)
	...
	!!-----------------------------------------------------------
 	...
 	use user_params,            only: change_default_values,exact_soln
    ...
    case(1)        !! ics for u [m/s]
		do k=1,nz
      		do i=1,nx
       			do j=1,ny
       				F(j,i,k) = exact_soln(x(i),y(j),z(k),t0,id)
       			enddo
       		enddo
       	enddo
...
subroutine user_bvals_EW(x,y,z,t,id,VALS,DERIVS,nx,ny,nz,side,Lx)
	...
	!   calling routine.
	!-----------------------------------------------------------------------------
	use user_params,                    only: exact_soln
	!-----------------------------------------------------------------------------
	...
	if( do_east )  then
		...
		do k=1,nz
			...
			do j=1,ny
				yval = y(j)
				VALS(j,k) = exact_soln(xval,yval,zval,t,id)
...
subroutine user_bvals_NS(x,y,z,t,id,VALS,DERIVS,nx,ny,nz,side,Ly)
	...
	if( do_south )  then
		...
		do k=1,nz
			...
			do i=1,nx
				xval = x(i)
				VALS(i,k) = exact_soln(xval,yval,zval,t,id)
...
subroutine user_bvals_BT(x,y,z,t,id,VALS,DERIVS,nx,ny,nz,side,Lz)
	...
	if( do_bottom )  then
		...
		do i=1,nx
			...
			do j=1,ny
				yval = y(j)
				VALS(i,j) = exact_soln(xval,yval,zval,t,id)

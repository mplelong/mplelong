subroutine decomposition2d
  ...
  use independent_variables,    only: nx,ny,nz,   z
  ...
  do pid=0,np-1
   ...
   proc_col(YBLOCK,pid) = floor( dfloat(pid)/dfloat(p1) + 1.d-6)
  ...
  do pid=0,np-1  
   ...
   if(pid==myid) then
    ...
    allocate( jk_indices_YBLOCK(npts,2) ) ; jk_indices_YBLOCK(:,:)=0
    do i=1,npts
   ...
   if(pid==myid) then
    ...
    allocate( ijk_indices_YBLOCK(npts,3) ) ; ijk_indices_YBLOCK(:,:)=0
    do ii=1,npts
  ...
  do pid=0,np-1  
   ...
   if( pid==myid ) then
    ...
    allocate( ijk_indices_ZBLOCK(npts,3) ) ; ijk_indices_ZBLOCK(:,:)=0
    do ii=1,npts

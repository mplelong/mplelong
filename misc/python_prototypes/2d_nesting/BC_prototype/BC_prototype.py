...
"""
  ...
  (4) some additional "fixes" made to help impose correct boundary behavior of
  	  tangential flow
  ...
  variable ids = [0,1,2,3] for [u,v,w,b] respectively used for RHS arrays
"""
...
from solver_utilities import apply_bcs, time_step,   \
                             divergence, div_ustar, grad, step, parent_soln, \
...
#------------------------------------------------------------
nt = 256                         # [1] time steps per wave period  unstable at 129, 257 seems ok  1024
...
#-----------------------------------------------------------------------------
# parameters for high order diffusion... use these even for Laplacian w/ p=1
#-----------------------------------------------------------------------------
...
#------------------------------------------------------------
Q = 5        # number of terms in Bernoulli polynomial series
...
#--------------------------------------------------------------------------
# set width of discrete step functions in grid points
...
print(" setup done, about to start time stepping loop ")
#---------------------------------------------------------------------------
...
for n in range(0,nsteps):
	...
	if(n==0): print(" evaluated gradients using BC method ")
	...
	if(n==0): print(" explicit time stepping done ")
	...
	#-----------------------------------------------------------------------------
	[divstar,ustar,wstar] = div_ustar(ustar,wstar,x,z,frac,LU_x,LU_z,Q,basis_functions,BVALS,STEP)
	if(n==0): print(" divergence of augmented ustar done ")	
	...
	if(n==0): print(" poisson inversion for phi done ");
	...
	if(n==0): print(" compute grad phi q/ Q=0 done ")
	...
	v = vstar
	w = wstar - dt*phi_z
	if(n==0): print(" pressure projection step done ")
	...
	if(n==0): print(" boundary conditions done ")
	...
	if(n==0): print(" diffusion via integrating factor done ")	
	...
	if( do_ncwrite ):	
		...
		#------------------------------------------------------------------------------
		# Output everything to a netcdf file
		#------------------------------------------------------------------------------	
		...
		success = write_netcdf_2d(vars,x,z,tnp1,fname,varnames,dims) 

	if(n==0): print(" 1st time step done ")

...
"""
    ...
    Arguments needed are: (1) top directory name, (2) and (3) p1 and p2, (4) name of file in top_dir+/XYZ/ directory
    ...
    Modified by P. Lelong (October 2021) to include s1_bar array in all restart files and, 
    if needed also s2_bar
. 
    This is needed if s1_bar(1-3)/s2_bar(1-3) are read from a file rather than defined in functional form. 
    The full z-profiles of s1_bar/s2_bar are included in all restart files. 
    
    Modified for x periodic, y and z closed intervals
"""
...
#---------------------------------------------------------------------------------------
#  number of x points per processor (periodic)
#--------------------------------------------------------------------------------------
locnx = int((nx)/p1) 
print("Assuming periodicity in x, locnx for all processors                  : ",locnx) 
...
for jproc in range(p2):
    ...
    for iproc in range(p1):
        ...
        if( do_second_scalar):
            ...
            s2_bar = f.createVariable('s2_bar','d',('nzglobal','ndim'))
        ...
        f.close() ; del u,v,w,s1,s1_bar
        if( do_second_scalar): del s2,s2_bar

subroutine InitialConditions  
	...
	use independent_variables,  only: x,y,z,t0,tf,Lz
	...
	use mpi_params
	use user_params,            only: rho_north,rho_south,NS_profiles
	implicit none 
	...
	allocate( rho_north(locnz), rho_south(locnz)  )  ! declared & saved in user_params module
	rho_north = 0.d0 ; rho_south = 0.d0 
	
	!--------------------------------------------------------------------------------------
	! set up the N/S profiles here rather than in user_ics because it doesn't
	! gat called when the run is a restart
	!--------------------------------------------------------------------------------------
   	do k=1,locnz
    	call NS_profiles(my_zvals(k),rho_north(k),rho_south(k))      ! N/S total density [kg/m3], local arrays
   	enddo
   
		!----------------------------------------------------------------------------------
		!   adjust top and bottom density values to approximately satisfy d/dz rho = 0 
		!   3 point stencil, A&S p 914, 1st deriv
		!----------------------------------------------------------------------------------
		k=1
		if( my_zvals(k)==0.d0 ) then
			rho_south(k) = (4.d0*rho_south(k+1) - rho_south(k+2))/3.d0
			rho_north(k) = (4.d0*rho_north(k+1) - rho_north(k+2))/3.d0
		endif
     
		k=locnz
		if( my_zvals(k)==Lz ) then
			rho_south(k) = (4.d0*rho_south(k-1) - rho_south(k-2))/3.d0
			rho_north(k) = (4.d0*rho_north(k-1) - rho_north(k-2))/3.d0
		endif

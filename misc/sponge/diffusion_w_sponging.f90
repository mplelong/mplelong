...
subroutine diffuse
	...
	use sponge,                       only: FW,sponge_filter_x,sponge_filter_y,sponge_filter_z ! FW=weighting function for band-pass filtered fields
 	implicit none
	...
	do id=1,nvars
		...
		!------------------------------------------------------------------------
		!  if sponging, apply x and y band pass filters to transformed field
		!  store result in tmpY(:,:,:,2)
		!------------------------------------------------------------------------
		if( do_sponging ) then
			do k=1,locnz
				do i=1,locnx
					ig = global_x_indices(START,YBLOCK,myid) + i - 1
					do j=1,ny
						tmpY(j,i,k,2) = tmpY(j,i,k,1)*sponge_filter_x(ig)*sponge_filter_y(j)
					enddo
				enddo
			enddo
		endif
		...
		if( do_sponging ) then    ! tmpY(:,:,:,2) has xy-transformed, xy-filtered idth field 
		                          ! tmpZ is now available
		
			!-------------------------------
			! transpose field id to ZBLOCK
			!-------------------------------
			call YBLOCK_2_ZBLOCK(tmpY(1,1,1,2),tmpZ)
	
			!-------------------------------------------
			! at each kx,ky, transform in z, 
			! apply z sponge filter, inverse transform
			!-------------------------------------------
			do i=1,array_size(JDIM,ZBLOCK,myid)
				do j=1,array_size(KDIM,ZBLOCK,myid)
					call z_sponge(tmpZ(1,i,j,1),tmpZ(1,i,j,2),nz)
				enddo
			enddo
			
			!-------------------------------
			! transpose back to YBLOCK
			!-------------------------------
			call ZBLOCK_2_YBLOCK(tmpZ(1,1,1,2),tmpY(1,1,1,1))
			
			!------------------------------------------------------------------------
			!  inverse transform in x and y, store filtered field in tmpY(:,:,:,2)
			!------------------------------------------------------------------------
			dir = -1  ! INV transforms (kx,ky) -> (x,y)
			call transform_xy(tmpY(1,1,1,1),tmpY(1,1,1,2),dir,exp_type)
			
			!------------------------------------------------------------------------
			!  take linear combination of filtered and unfiltered fields such that
			!  the result is filtered w/in the sponge layers and unfiltered at the
			!  boundaries and in the bulk of the interior domain
			!------------------------------------------------------------------------
			if( id==1 ) u(:,:,:)  = (1.d0 - FW(:,:,:))*u(:,:,:)  + FW(:,:,:)*tmpY(:,:,:,2)
			if( id==2 ) v(:,:,:)  = (1.d0 - FW(:,:,:))*v(:,:,:)  + FW(:,:,:)*tmpY(:,:,:,2)
			if( id==3 ) w(:,:,:)  = (1.d0 - FW(:,:,:))*w(:,:,:)  + FW(:,:,:)*tmpY(:,:,:,2)
			if( id==4 ) s1(:,:,:) = (1.d0 - FW(:,:,:))*s1(:,:,:) + FW(:,:,:)*tmpY(:,:,:,2)
			if( id==5 ) s2(:,:,:) = (1.d0 - FW(:,:,:))*s2(:,:,:) + FW(:,:,:)*tmpY(:,:,:,2)
...
subroutine z_sponge(f,ans,nz)
!--------------------------------------------------!
!                                                  !
!     Given f(z) take z transform,  apply the      !
!     z band pass sponging filter, and             !
!     inverse transform, returning the filtered    !
!     field in the array ans.                      !
!--------------------------------------------------!
	use differentiation_params, only: cos_plan  ! cos_plan(3) in x,y,z dirs
	use sponge,                 only: sponge_filter_z
	implicit none
	...
	!----------------------------------------------
	!  multiply by the band pass filter and
	!  normalize the result, overwriting wrk(:)
	!----------------------------------------------
	do k=1,nz
		wrk(k) = xnorm * sponge_filter_z(k) * wrk(k)
	enddo
